...
def apply_watermark(
    ...
    cmd = [
        ...
        "fastdecode",
        # "-filter_complex",
        # f"[1]format=rgba,colorchannelmixer=aa={wtm.opacity},scale=iw*{wtm.width}:-1[logo];[0][logo]overlay={wtm.offset}, drawtext=fontfile=/path/to/font.ttf:text='Stack Overflow':fontcolor=white:fontsize=24:box=1:boxcolor=black@0.5:boxborderw=5:x=(w-text_w)/2:y=(h-text_h)/2",
        "-filter_complex",
        f"[1]format=rgba,colorchannelmixer=aa={wtm.opacity},rotate={wtm.rotate}*PI/180:c=black@0:ow=rotw({wtm.rotate}*PI/180):oh=roth({wtm.rotate}*PI/180),scale=iw*{wtm.width}:-1[logo];[0][logo]overlay={wtm.offset}",
